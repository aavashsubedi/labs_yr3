...
def convert_2d_hist(values, x_bins_edges, y_bins_edges, x_resolution=200, limits_x=[0, 20]):
    ...
    for row in range(0, len(values)):
        ...
        plt.plot([x_bins_edges[row][0], x_bins_edges[row][-1]], [y_bins_edges[row + 1], y_bins_edges[row + 1]], color='r', ls='--', lw='1')
        plt.plot([x_bins_edges[row][0], x_bins_edges[row][0]], [y_bins_edges[row], y_bins_edges[row + 1]], color='r', ls='-', lw='1')
        for column in range(0, len(values[row])):
            ...
            for iterator in range(0, len(histogram_frame)):
                ...
                if histogram_frame[iterator] >= x_bins_edges[row][column + 1]:
                    plt.plot([x_bins_edges[row][column + 1], x_bins_edges[row][column + 1]], [y_bins_edges[row], y_bins_edges[row + 1]], color='r', ls='-', lw='1')
                    break
